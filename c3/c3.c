...
int main()
{
    ...
    status = pthread_create(&producer_tid, NULL, producer, NULL); // 생산자 쓰레드 생성
    if (status != 0)
    {
        perror("Create thread");
    }
    status = pthread_create(&consumer_tid, NULL, consumer, NULL); // 소비자 쓰레드 생성
    if (status != 0)
    {
        perror("Create thread");
    }

    status = pthread_join(producer_tid, NULL); // 쓰레드 결합
    if (status != 0)
    {
        perror("Join producer thread");
    }
    status = pthread_join(consumer_tid, NULL); // 쓰레드 결합
    if (status != 0)
    {
