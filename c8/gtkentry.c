...
void input_completed(GtkWidget *entry, gpointer data)
{
    ...
    if (strcmp(text, password) == 0)
        gtk_label_set_text(GTK_LABEL((GtkWidget *)data),
                           "로긴이 허용됩니다!\n");
    else
        gtk_label_set_text(GTK_LABEL((GtkWidget *)data),
                           "입력 오류입니다!\n");
...
int main(int argc, char *argv[])
{
    ...
    gtk_box_pack_start(GTK_BOX(user_hbox), user_entry,
                       TRUE, FALSE, 5);
    gtk_box_pack_start(GTK_BOX(passwd_hbox),
                       passwd_label, TRUE, FALSE, 5);
    gtk_box_pack_start(GTK_BOX(passwd_hbox),
                       passwd_entry, TRUE, FALSE, 5);
    gtk_box_pack_start(GTK_BOX(vbox), user_hbox, FALSE,
                       FALSE, 10);
    gtk_box_pack_start(GTK_BOX(vbox), passwd_hbox,
                       FALSE, FALSE, 10);
