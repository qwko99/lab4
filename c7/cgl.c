...
void send_msg_get(int sockfd);
void send_msg_post(int sockfd);

int main(int argc, char *argv[])
{
    int sockfd, new_fd;             /* listen on sock_fd, new connection on new_fd */
    struct sockaddr_in server_addr; /* structure to old server's address */
...
void *handle_clnt(int client_sock)
{
    ...
    else if (strcmp(method, "GET") == 0) // GET
    {
        send_msg_get(client_sock);
    }
    else if (strcmp(method, "POST") == 0) // POST
    {
        send_msg_post(client_sock);
    }
    else
        send_err(client_sock);
    return NULL;
...
void send_msg_get(int client_sock) // send to all
{
    ...
    char contenttype[] = "Content-Type:text/html\r\n";
    char html[] = "<html><head>GET TEST</ head><body><H1>GET MODE</ H1></ body></ html>\r\n ";
    char end[] = "\r\n";
...
void send_msg_post(int client_sock) // send to all
{
    char protocol[] = "HTTP/1.1 200 OK\r\n";
    char server[] = "Server:Netscape-Enterprise/6.0\r\n";
    char contentlength[] = "Content-Length: 13\r\n";
    char contenttype[] = "Content-Type:application/x-www-form-urlencoded\r\n";
    char html[] = "<html><head>POST TEST</ head><body><H1>POST MODE</ H1></ body></ html>\r\n ";
    char end[] = "\r\n";
    printf("send msg, len=%d\n", strlen(html));
    write(client_sock, protocol, strlen(protocol));
    write(client_sock, server, strlen(server));
